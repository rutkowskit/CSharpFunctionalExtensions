...
public static partial class ResultExtensions
{
    ...
    /// </summary>
    /// T
    public static K Match<K, T>(this Result<T> result, Func<T, K> onSuccess, Func<IError, K> onFailure)
    {
        return result.IsSuccess
            ? onSuccess(result.Value)
            : onFailure(result.Error);
    ...
    /// </summary>
    /// T
    public static T Match<T>(this Result result, Func<T> onSuccess, Func<IError, T> onFailure)
    {
        return result.IsSuccess
            ? onSuccess()
            : onFailure(result.Error);
    ...
    ///     Invokes the given <paramref name="onSuccess"/> action if the calling Result is a success. Otherwise, it invokes the given <paramref name="onFailure"/> action.
    /// </summary>
    public static void Match<T>(this Result<T> result, Action<T> onSuccess, Action<IError> onFailure)
    {
        if (result.IsSuccess)
            onSuccess(result.Value);
        else
    ...
    /// </summary>
    /// T
    public static void Match(this Result result, Action onSuccess, Action<IError> onFailure)
    {
        if (result.IsSuccess)
            onSuccess();
        else
