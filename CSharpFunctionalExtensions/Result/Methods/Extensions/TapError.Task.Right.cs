...
public static partial class ResultExtensions
{
    ...
    ///     Executes the given action if the calling result is a failure. Returns the calling result.
    /// </summary>
    public static async Task<Result> TapError(this Result result, Func<IError, Task> func)
    {
        if (result.IsFailure)
        {
    ...
    ///     Executes the given action if the calling result is a failure. Returns the calling result.
    /// </summary>
    public static async Task<Result<T>> TapError<T>(this Result<T> result, Func<IError, Task> func)
    {
        if (result.IsFailure)
        {
            await func(result.Error).DefaultAwait();
